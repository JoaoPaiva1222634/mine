import { separateEvenAndOddNumbers } from "../../lab03/ex16";

describe('testing function separateEvenAndOddNumbers', () => {
    test('testing if for correct number is true', () => {
        //Arrange
        const numberSequence: number[] = [1, 2, 3, 4, 5, 6, 7, 8, 9];
        const expectedResult: number[] = [1, 3, 5, 7, 9, 2, 4, 6, 8];

        //Act
        let result: number[] = separateEvenAndOddNumbers(numberSequence);

        //Assert
        for (let i = 0; i < expectedResult.length; i++) {
            expect(result[i]).toBe(expectedResult[i]);
        }
    });

    test('testing if for correct number is true', () => {
        //Arrange
        const numberSequence: number[] = [1, 3, 5, 7, 9];
        const expectedResult: number[] = [1, 3, 5, 7, 9];

        //Act
        let result: number[] = separateEvenAndOddNumbers(numberSequence);

        //Assert
        for (let i = 0; i < expectedResult.length; i++) {
            expect(result[i]).toBe(expectedResult[i]);
        }
    });

    test('testing if for correct number is true', () => {
        //Arrange
        const numberSequence: number[] = [-9];
        const expectedResult: number[] = [-9];

        //Act
        let result: number[] = separateEvenAndOddNumbers(numberSequence);

        //Assert
        for (let i = 0; i < expectedResult.length; i++) {
            expect(result[i]).toBe(expectedResult[i]);
        }
    });

    /*
    test('For values not above 0 should throw error', () => {
        expect(() => {
            separateEvenAndOddNumbers([11, 1, 2, 3, 4, 5, 6, -9]);
        }).toThrow('Numbers in sequence must have only one digit');
    });

    test('For values not above 0 should throw error', () => {
        expect(() => {
            separateEvenAndOddNumbers([-11, 1, 2, 3, 4, 5, 6, 7]);
        }).toThrow('Numbers in sequence must have only one digit');
    });
    */
});